...
int main() {
    ...
    while (true) {
        ...
        for (const auto& drone : drones) {
            cv::circle(image, cv::Point2d(drone.x * 4 + imageSize / 2, drone.y * 4 + imageSize / 2), 5, cv::Scalar(0, 255, 0), -1);
        }

        // 绘制起点和目标点
        cv::circle(image, cv::Point2d(startPoint.x * 4 + imageSize / 2, startPoint.y * 4 + imageSize / 2), 10, cv::Scalar(0, 0, 255), -1);
        cv::circle(image, cv::Point2d(targetPoint.x * 4 + imageSize / 2, targetPoint.y * 4 + imageSize / 2), 10, cv::Scalar(255, 0, 0), -1);
...
```
![image](https://github.com/LQepoch/Robotic_Fish_Matlab_Test/assets/43512109/ad57641f-d873-434c-96e2-72855d4eb0df)
